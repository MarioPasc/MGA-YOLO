...
from ultralytics.models import yolo  # required for references elsewhere in file
...
class YOLO(Model):
    ...
    def __init__(self, model: Union[str, Path] = "yolo11n.pt", task: Optional[str] = None, verbose: bool = False):
        ...
        # =========== MGA ADAPTATION HERE ===========
        # If model path or YAML content indicates MGA (contains 'mga' in name or MGAMaskHead layer), force task='mga'
        if path.suffix in {".pt", ".yaml", ".yml"}:
            if "mga" in path.stem.lower():
                task = "mga"
            else:
                if path.suffix in {".yaml", ".yml"}:
                    try:
                        txt = path.read_text()
                        if "MGAMaskHead" in txt:
                            task = "mga"
                    except Exception:
                        pass
    ...
    def task_map(self) -> Dict[str, Dict[str, Any]]:
        ...
        # Local imports to avoid circular dependency during package initialization
        from ultralytics.models.yolo import classify, detect, segment, pose, obb  # type: ignore
        base_map = {
            "classify": {
